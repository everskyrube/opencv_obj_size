...
ap.add_argument("-w", "--width", type=float, required=False, default=0.955, help="width of the left-most object in the image (in inches)")
...
for c in cnts:
    ...
    if pixelsPerMetric is None:              # if the pixels_per_metric is not initialised
        pixelsPerMetric = dB / args["width"] # compute it as the ratio of pixels to input "width" argument

    dimA = dA / pixelsPerMetric # compute two physical dimensions of height & width of the minAreaRect
    dimB = dB / pixelsPerMetric
    # draw the object sizes on the image
    cv2.putText(orig, "{:.1f} inch".format(dimA), (int(tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
    cv2.putText(orig, "{:.1f} inch".format(dimB), (int(trbrX + 10), int(trbrY)), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (255, 255, 255), 2)
